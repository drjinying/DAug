...
def main():
    ...
    ###############################################
    args.resume_checkpoint = '/yjblob/medical/ckpt/diff_classifier_multi/ep_008.pt'
    ###############################################

    if args.resume_checkpoint:
        resume_step = 9 #parse_resume_step_from_filename(args.resume_checkpoint)
        if dist.get_rank() == 0:
            logger.log(
    ...
    for epoch in range(resume_step, args.iterations):
        ...
        if args.anneal_lr:
            set_annealed_lr(opt, args.lr, (epoch + resume_step) / args.iterations)
